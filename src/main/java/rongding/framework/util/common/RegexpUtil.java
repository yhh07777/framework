...
/**
 * 类简介: 使用正则表达式验证数据或提取数据,类中的方法全为静态的 主要方法:1. isHardRegexpValidate(String source,
 * String regexp) 区分大小写敏感的正规表达式批配 * 2. isSoftRegexpValidate(String source,
 * String regexp) 不区分大小写的正规表达式批配 3. getHardRegexpMatchResult(String source,
 * String regexp) 返回许要的批配结果集(大小写敏感的正规表达式批配) 4. getSoftRegexpMatchResult(String
 * source, String regexp) 返回许要的批配结果集(不区分大小写的正规表达式批配) 5 getHardRegexpArray(String
 * source, String regexp) 返回许要的批配结果集(大小写敏感的正规表达式批配) 6.
 * getSoftRegexpMatchResult(String source, String regexp)
 * 返回许要的批配结果集(不区分大小写的正规表达式批配) 7. getBetweenSeparatorStr(final String
 * originStr,final char leftSeparator,final char rightSeparator)
 * 得到指定分隔符中间的字符串的集合
 */
