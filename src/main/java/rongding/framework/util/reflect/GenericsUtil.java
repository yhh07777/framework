...
public class GenericsUtil {
	...
	/**
	 * 获取指定类的父类的泛型参数的实际类型名称
	 * @param clazz 需要反射的类,该类必须继承范型父类
	 * @param index 泛型参数所在索引,从0开始.
	 * @return 类的完整路径名称
	 */
	public static String getSuperClassGenricName(Class clazz) {
		return getSuperClassGenricType(clazz,0).getName();
	}
	public static String getSuperClassGenricName(Class clazz,int index) {
		return getSuperClassGenricType(clazz,index).getName();
	}
	/**  
	...
	public static Class getMethodGenericReturnType(Method method, int index) {
    	...
    	//如果实现ParameterizedType接口，即支持泛型 
    	if(returnType instanceof ParameterizedType){
    		//强制转化为ParameterizedType
